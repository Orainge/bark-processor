package com.orainge.bark_processor.server.forward.server.websocket;

import com.alibaba.fastjson.JSON;
import com.orainge.bark_processor.server.forward.server.config.WebSocketServerConfigurator;
...
public class WebsocketServerEndpoint {
    ...
    public void onOpen(Session session, EndpointConfig config) {
        ...
        if (this.clientConfig == null) {
            try {
                log.error("[Bark Processor 服务端] - 无法获取用户的客户端信息，关闭连接");
                session.close();
        ...
        if (!addSuccess) {
            ...
            try {
                ...
                log.error("[Bark Processor 服务端] - 已有已有相同 ID [{}] 的客户端连接", JSON.toJSONString(this.clientConfig));
                // 关闭连接
                session.close();
            } catch (Exception ignore) {
            }
        ...
        log.info("[Bark Processor 服务端] - 客户端 [{}] 连接成功", JSON.toJSONString(this.clientConfig));
    }
    ...
    public void onClose() {
        ...
        if (this.clientConfig != null) {
            log.info("[Bark Processor 服务端] - 客户端 [{}] 断开连接", JSON.toJSONString(this.clientConfig));
        }
    }
    ...
    public void onError(Session session, Throwable error) {
        if (error instanceof EOFException) {
            ...
            log.error("[Bark Processor 服务端] - 连接异常", error);
        }
    }

package com.orainge.bark_processor.server.forward.server.util;
...
import com.orainge.bark_processor.server.forward.server.websocket.ClientResponseContainer;
import com.orainge.bark_processor.server.forward.server.websocket.WebsocketServerEndpoint;
import com.orainge.bark_processor.server.forward.server.websocket.WebsocketServerEndpointContainer;
...
public class WebsocketServerUtil {
    ...
    public boolean add(WebsocketServerEndpoint server) {
        ...
        if (isSuccess) {
            log.info("[Bark Processor 服务端] - 当前连接数: {}", WebsocketServerEndpointContainer.size());
            return true;
        } else {
            return false;
        }
    ...
    public void onMessage(String clientId, Session session, String message) {
        if (MessageCode.KEEP_ALIVE_TAG.equals(message)) {
            ...
            try {
                ...
                log.debug("[Bark Processor 服务端] 发送心跳响应包成功 [ID: " + clientId + "]");
                return;
            } catch (Exception e) {
                log.error("[Bark Processor 服务端] 发送心跳响应包失败 [ID: " + clientId + "]", e);
                return;
            }
        }
        ...
        if (MessageCode.KEEP_ALIVE_TAG_RESPONSE.equals(message)) {
            // 客户端对服务端心跳包的回应，不做处理
            log.debug("[Bark Processor 服务端] 收到来自客户端的心跳检测响应 [ID: " + clientId + "]");
            return;
        }
    ...
    public Result send(String clientId, ExchangeMessage<?> exchangeMessage) {
        try {
            ...
            if (server == null) {
                ...
                log.error("[Bark Processor 服务端] - " + tips);
            ...
            log.error("[Bark Processor 服务端] - " + tips, e);
            return Result.error().setMessage(tips);
        }
    }
...
//                    if (endpoint != null) {
                        ...
                        log.debug("[Bark Processor 服务端] 发送心跳检测包成功 - [ID: " + clientId + "]");
                    }
                } catch (Exception e) {
                    log.error("[Bark Processor 服务端] 发送心跳检测包失败 - [ID: " + clientId + "]", e);
